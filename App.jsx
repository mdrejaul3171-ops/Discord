...
export default function App() {
  ...
  useEffect(() => {
    if (view === 'dashboard-view' && chartRef.current && window.Chart) {
      ...
      chartInstanceRef.current = new window.Chart(chartRef.current.getContext('2d'), { type: 'line', data: { labels: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'], datasets: [{ label: 'Revenue (₹)', data: chartData, borderColor: '#c5a880', backgroundColor: 'rgba(197, 168, 128, 0.2)', borderWidth: 3, fill: true, tension: 0.4 }] }, options: { responsive: true, plugins: { legend: { display: false } }, scales: { x: { ticks:{color:textColor}, grid:{display:false} }, y: { ticks:{color:textColor}, grid:{color: 'rgba(200,200,200,0.1)'} } } } });
    }
      ...
      <div className="main-content">
        ...
        {view === 'dashboard-view' && (
          <div className="view-section active">
            ...
            <div className="card" style={{cursor: 'pointer'}} onClick={() => setView('analytics-view')}><div className="card-header"><h3>Revenue Analytics <span style={{fontSize:'12px', color:'#888', fontWeight:'normal'}}>(Click for details)</span></h3></div><div style={{padding: '20px'}}><canvas ref={chartRef} height="80"></canvas></div></div>
          </div>
        )}

        {view === 'analytics-view' && (
          <div className="view-section active"><div className="card"><div className="card-header"><h3>Detailed Analytics Report</h3><button className="btn-primary" onClick={fetchData}>Refresh Data</button></div><div className="form-grid">
            ...
            {Object.keys(products).reverse().map(key => {
              ...
              return (
                <tr key={key}>
                  ...
                  <td>
                    <div style={{display:'flex', gap:'5px', marginBottom:'5px'}}><input type="number" value={stockInputs[key] ?? ''} onChange={e=>setStockInputs({...stockInputs, [key]: e.target.value})} style={{width:'60px', padding:'5px', border:'1px solid var(--border-color)', borderRadius:'4px', background:'var(--input-bg)', color:'var(--text-main)'}} /><button onClick={() => updateStock(key)} style={{background:'#3699ff', color:'white', border:'none', padding:'6px 10px', borderRadius:'4px', cursor:'pointer'}}><i className="fas fa-save"></i></button></div>
        ...
        {view === 'chat-view' && (
          <div className="view-section active"><div className="card" style={{marginBottom:0}}><div className="card-header"><h3>💬 Customer Support</h3><button className="btn-primary" onClick={fetchChatList}>Refresh</button></div><div className="chat-layout">
            <div className="chat-list">{Object.keys(chats).length === 0 ? <p style={{padding:'15px', color:'#888', textAlign:'center'}}>No active chats</p> : Object.keys(chats).map(uid => (<div key={uid} className={`chat-user-item ${activeChatUserId === uid ? 'active' : ''}`} onClick={() => {setActiveChatUserId(uid); fetchAdminMessages(uid);}}><b><i className="fas fa-user-circle"></i> {chats[uid].userName || 'Customer'}</b></div>))}</div>
            <div className="chat-window">
              ...
              <div className="chat-messages">{!activeChatUserId ? <div style={{height:'100%', display:'flex', alignItems:'center', justifyContent:'center', color:'#888'}}>Select a customer</div> : Object.keys(chatMessages).map(k => { let m = chatMessages[k]; return <div key={k} className={`msg-bubble ${m.sender === 'admin' ? 'msg-admin' : 'msg-user'}`}>{m.text}</div>; })}{activeChatUserId && <div ref={chatEndRef} />}</div>
      ...
      {modalType === 'order-details' && modalData && (
        <div className="modal-overlay" style={{display:'flex', position:'fixed', inset:0, width:'100vw', height:'100vh', backgroundColor:'rgba(0,0,0,0.85)', zIndex:999999, alignItems:'center', justifyContent:'center'}}>
          <div className="modal-content" style={{maxWidth:'500px', width:'90%', maxHeight:'85vh', overflowY:'auto', backgroundColor:'var(--card-bg)', padding:'25px', borderRadius:'12px', border:'1px solid var(--border-color)', position:'relative'}}>
            <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', borderBottom:'1px solid var(--border-color)', paddingBottom:'15px', marginBottom:'20px'}}><h2 style={{fontFamily:'Playfair Display', color:'var(--primary)', margin:0}}>📦 Shipping Details</h2><button onClick={()=>setModalType(null)} style={{background:'none', border:'none', fontSize:'24px', cursor:'pointer', color:'#f64e60', padding:0}}><i className="fas fa-times-circle"></i></button></div>
            <div style={{fontSize:'15px', lineHeight:1.6}}>
              <div style={{background:'var(--input-bg)', padding:'15px', borderRadius:'8px', marginBottom:'15px', border:'1px solid var(--border-color)'}}><h4 style={{color:'var(--primary)', margin:'0 0 10px 0'}}><i className="fas fa-user"></i> Customer Info</h4><p style={{margin:0}}><b>Name:</b> {modalData.customerName || 'N/A'}</p><p style={{margin:0}}><b>Phone:</b> <a href={`tel:${modalData.phone}`} style={{color:'#3699ff', textDecoration:'none'}}>{modalData.phone || 'N/A'}</a></p></div>
              <div style={{background:'var(--input-bg)', padding:'15px', borderRadius:'8px', marginBottom:'15px', border:'1px solid var(--border-color)'}}><h4 style={{color:'var(--primary)', margin:'0 0 10px 0'}}><i className="fas fa-map-marker-alt"></i> Delivery Address</h4><p style={{margin:0}}>{modalData.address || 'N/A'}</p></div>
              <div style={{background:'var(--input-bg)', padding:'15px', borderRadius:'8px', marginBottom:'15px', border:'1px solid var(--border-color)'}}><h4 style={{color:'var(--primary)', margin:'0 0 10px 0'}}><i className="fas fa-shopping-bag"></i> Order Summary</h4><p style={{margin:0}}><b>Items:</b> {typeof modalData.items === 'string' ? modalData.items : JSON.stringify(modalData.items)}</p>
                {(() => { try { if (typeof modalData.items !== 'string') return null; let itemNames = modalData.items.replace(/\[.*?\]/g, '').split(',').map(s=>s.trim().toLowerCase()); let matched = Object.values(products).filter(p => p && p.name && itemNames.some(i => i.includes(String(p.name).toLowerCase()))); if (matched.length === 0) return null; return (<div style={{display:'flex', gap:'10px', marginTop:'10px', overflowX:'auto', paddingBottom:'5px'}}>{matched.map((p,i)=>(<div key={i} style={{textAlign:'center', minWidth:'70px'}}><a href={p.img} target="_blank" rel="noreferrer"><img src={p.img} style={{width:'60px', height:'60px', objectFit:'cover', borderRadius:'6px', border:'2px solid var(--primary)'}} alt=""/></a><p style={{fontSize:'10px', fontWeight:'bold', margin:'4px 0 0 0', maxWidth:'70px', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>{p.name}</p></div>))}</div>); } catch(e) { return null; } })()}
                <p style={{margin:'10px 0 0 0'}}><b>Total Amount:</b> ₹{modalData.totalAmount || 0}</p><p style={{margin:0}}><b>Payment Mode:</b> <span style={{background:'#333', color:'white', padding:'4px 8px', borderRadius:'4px', fontSize:'11px', textTransform:'uppercase'}}>{modalData.paymentType || 'COD'}</span></p>
              </div>
              {modalData.paymentType === 'UPI' && modalData.upiScreenshot && (<div style={{background:'var(--input-bg)', padding:'15px', borderRadius:'8px', marginBottom:'15px', border:'1px solid var(--border-color)', textAlign:'center'}}><h4 style={{color:'#6528F7', margin:'0 0 10px 0'}}><i className="fas fa-receipt"></i> Payment Screenshot</h4><img src={modalData.upiScreenshot} style={{width:'100%', maxWidth:'250px', borderRadius:'8px', border:'2px solid #ddd', cursor:'pointer'}} onClick={()=>window.open(modalData.upiScreenshot)} alt=""/></div>)}
              {modalData.status === 'Pending' && (<button type="button" onClick={async ()=>{await updateOrder(modalData.id, 'status', 'Accepted'); setModalType(null);}} style={{width:'100%', padding:'15px', marginTop:'15px', background:'#1bc5bd', color:'white', border:'none', borderRadius:'8px', fontWeight:'bold', fontSize:'16px', cursor:'pointer'}}><i className="fas fa-check-circle"></i> Accept Order</button>)}
            </div>
          </div>
        </div>
      )}
